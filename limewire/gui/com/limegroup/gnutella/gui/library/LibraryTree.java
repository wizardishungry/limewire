   ...
   // private static final Log LOG = LogFactory.getLog(LibraryTree.class);
	...
	public void handleFileManagerEvent(final FileManagerEvent evt) {
        ...
        case ADD_STORE_FOLDER: 
            File[] storeFiles = evt.getFiles();
            addDirectoryToNode(storeFiles[0], lwsFilesNode, true);
            break;
        case REMOVE_STORE_FOLDER: 
            File removedFolder = evt.getFiles()[0];
            removeFolder(removedFolder, lwsFilesNode);
            break;
        case ADD_STORE_FILE: 
            if(lwssfh.accept( evt.getFileDescs()[0].getFile())){
	...
	private boolean canBeShared(LibraryTreeNode node) {
		if (node == null || node == incompleteFilesNode || node == torrentsMetaFilesNode || node == lwsFilesNode ||
                node == lwsSpeciallFilesNode)
			return false;
	...
	private void showIncompleteFolderMessage() {
		GUIMediator.showError(I18n.tr("LimeWire will not allow you delete to the folder reserved for incomplete files."));
	}
	...
	private void makePopupMenu() {
		...
		DIRECTORY_POPUP.add(new JMenuItem(new ConfigureOptionsAction(
                ...
                I18n.tr("You can configure the folders you share in LimeWire\'s Options."))));
	}
